<?php
    ...
    $app->get("/", function() use ($app) {
        return $app['twig']->render('index.html.twig', array('category' =>
        Category::getAll(), 'tasks' => Task::getAll()));
    });
    ...
    $app->post("/add_categories", function () use ($app) {
        ...
        return $app['twig']->render('task.html.twig', array('task' => $task, 'tasks' => Task::getAll()
    ...
    $app->post("/tasks", function () use ($app) {
        $id = null;
        $task = new Task($_POST['name'], $id, $_POST['due_date']);
        $task->save();
        return $app['twig']->render('tasks.html.twig', array('tasks' => Task::getAll(), 'categories' => Category::getAll()));
    });
    $app->get("/tasks/{id}", function($id) use ($app) {
        $task = Task::find($id);
        return $app['twig']->render('task.html.twig', array('task' => $task, 'categories' =>
        $task->getCategories(),'all-categories' => Category::getAll()));
    });
    //deletes ALL tasks
    ...
    $app->post("/add_tasks", function() use ($app) {
        ...
        return $app['twig']->render('category.html.twig', array('category' => $category,
         'categories' => Category::getAll(), 'tasks' => $category->getTasks(), 'all_tasks' =>
         Task::getAll()));
    });
