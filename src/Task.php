...
class Task
{
    ...
    {
      $GLOBALS['DB']->exec("UPDATE tasks SET description =
      '{$new_description}', '{$new_due_date}' WHERE id = {$this->getId()};");
    ...
    {
        $GLOBALS['DB']->exec("INSERT INTO categories_tasks (category_id, task_id)
        VALUES ({$category->getId()}, {$this->getId()});");
    }
    function getCategories()
    {
        $query = $GLOBALS['DB']->query("SELECT * FROM categories WHERE id = {$category_id};");
